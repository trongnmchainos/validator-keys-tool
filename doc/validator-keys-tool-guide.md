...
change if the jbcoind config and/or server are compromised.
...
*   sign tokens authorizing a jbcoind server to run as the validator identified
    by this public key.
...
The current token needs to be present in the jbcoind config file.
...
Sample output:
```
  Validator keys stored in /home/ubuntu/.jbcoin/validator-keys.json
```
...
```
  Update jbcoind.cfg file with these values:
...
For a new validator, add the [validator_token] value to the jbcoind config file.
...
value. After the config is updated, restart jbcoind.
...
restart jbcoind. Rename the old key file and generate new [validator keys](#validator-keys) and
