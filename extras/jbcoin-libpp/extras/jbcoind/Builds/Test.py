...
CMAKE_UNITY_COMBOS = { '' : [['jbcoind', 'jbcoind_classic'], CMAKE_UNITY_CONFIGS],
    '.nounity' : [['jbcoind', 'jbcoind_unity'], CMAKE_NONUNITY_CONFIGS] }
...
def run_tests(args):
    failed = []
    if IS_WINDOWS:
        binary_re = re.compile(r'build\\([^\\]+)\\jbcoind.exe')
    else:
        binary_re = re.compile(r'build/([^/]+)/jbcoind')
