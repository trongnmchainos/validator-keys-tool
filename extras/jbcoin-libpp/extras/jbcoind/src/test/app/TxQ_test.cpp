...
public:
    void testQueue()
    {
        ...
        env.fund(XRP(50000), nojbcoin(alice, bob, charlie, daria));
        ...
        env.fund(XRP(50000), nojbcoin(elmo, fred, gwen, hank));
    ...
    void testLocalTxRetry()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(XRP(50000), nojbcoin(alice, bob, charlie));
    ...
    void testLastLedgerSeq()
    {
        ...
        env.fund(XRP(1000), nojbcoin(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(XRP(1000), nojbcoin(charlie, daria));
        env.close(env.now() + 5s, 10000ms);
        env.fund(XRP(1000), nojbcoin(edgar, felicia));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testZeroFeeTxn()
    {
        ...
        env.fund(XRP(1000), nojbcoin(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(XRP(1000), nojbcoin(carol));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testPreclaimFailures()
    {
        ...
        env.fund(XRP(1000), nojbcoin(alice));
    ...
    void testMultiTxnPerAccount()
    {
        ...
        env.fund(XRP(500000), nojbcoin(bob, charlie, daria));
    ...
    void testDisabled()
    {
        ...
        env.fund(XRP(50000), nojbcoin(alice));
    ...
    void testUnexpectedBalanceChange()
    {
        ...
        env.fund(XRP(50000), nojbcoin(bob));
    ...
    void testBlockers()
    {
        ...
        env.fund(XRP(50000), nojbcoin(alice, bob));
        env.memoize(charlie);
        env.memoize(daria);
    ...
    void testInFlightBalance()
    {
        ...
        env.fund(XRP(50000), nojbcoin(alice, charlie), gw);
        checkMetrics(env, 0, boost::none, 4, 3, 256);
    ...
    void testExpirationReplacement()
    {
        ...
        */
        ...
        env.fund(XRP(500000), nojbcoin(alice, bob));
        checkMetrics(env, 0, boost::none, 2, 1, 256);
    ...
    void testServerSubscribe()
    {
        ...
        env.fund(XRP(50000), nojbcoin(a,b,c,d));
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        env.fund(XRP(50000), nojbcoin(e,f,g,h,i));
...
BEAST_DEFINE_TESTSUITE(TxQ,app,jbcoin);
